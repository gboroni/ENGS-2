...
public interface Apresentacaos extends JpaRepository<Apresentacao, Long> {
	...
	@Modifying
	@Query(value="select a.id as id, a.id_tema as id_tema,a.status as status, a.inicio as inicio, a.fim as fim, t.nome as nome_tema, a.avaliacoes as avaliacoes  from tb_apresentacao a join tb_tema t on t.id = a.id_tema", nativeQuery=true)
	List<Apresentacao> findAllApresentacaos();
	
	@Modifying
	@Query(value="select a.id as id, a.id_tema as id_tema, a.inicio as inicio, a.fim as fim , t.nome as nome_tema,a.status as status, (select  count(DISTINCT avaliador)  from Tb_Avaliacao where id_apresentacao = a.id ) as avaliacoes from Tb_Apresentacao a JOIN Tb_Tema t on a.id_tema = t.id where (select  count( DISTINCT avaliador)  from Tb_Avaliacao where id_apresentacao = a.id ) > 0", nativeQuery=true)
